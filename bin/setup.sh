...
function create-project {
  ...
  <body>
    ...
    <script src="{% static 'mainController.js' %}" charset="utf-8"></script>
  </body>
  </html>
BASE

  # create static in project folder
  mkdir $PROJECT/static

  touch $PROJECT/static/app.js
  cat <<APP > $PROJECT/static/app.js
  var app = angular.module('myApp', []);
APP

  touch $PROJECT/static/mainController.js
  cat <<CONTROLLER > $PROJECT/static/mainController.js
...
CONTROLLER
  ...
  # start looking for static files
  # in project
  sed -i "s/^STATIC_URL/STATICFILES_DIRS = ( '${PROJECT}\/static\/', )\nSTATIC_URL/" $PROJECT/settings.py
}
function add-splash {
  local app=$SPLASH
